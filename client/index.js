...
async function pokeApi() {
    ...
    const battle = await getBattleSim([pokeInfo[0].name, pokeInfo[1].name], randMoves)
    console.log(battle)
...
function updatePokeHtml(pokeInfo) {
    ...
    document.getElementById('battle').innerText = 'Simulating... May take up to 20 seconds to simulate battle'
    ...
    // automatically setting the font size for the name div by setting up a dummy div to fit the font, and decreasing the font size until the dummy div is smaller than the name div, then setting the name at that font in the name div, this way it does not overfill ever
        ...
        const nameDivWidth = document.getElementById(`poke-name-${pokeIdx}`).clientWidth;
        ...
        let dummyDivWidth = document.getElementById(`dummy-font-test`).clientWidth;
        let initFont = 2.5;
        ...
        while(dummyDivWidth > (nameDivWidth * 0.9)) {
            ...
            dummyDivWidth = document.getElementById(`dummy-font-test`).clientWidth;
            console.log(dummyDiv.style.fontSize);
            console.log(document.getElementById(`dummy-font-test`).clientWidth);
        }
        console.log(initFont)
        console.log(document.getElementById(`dummy-font-test`).clientWidth)
        document.getElementById(`poke-name-${pokeIdx}`).style.fontSize = `${initFont}rem`

    // setting the pokemon name, sprite image, and background color for the card (based on first type)
        document.getElementById(`poke-name-${pokeIdx}`).innerText = capName
        document.getElementById(`poke-sprite-${pokeIdx}`).src = pokemon.sprites.front_default
        document.getElementById(`poke-card-${pokeIdx}`).style.backgroundColor = colors[pokemon.types[0].type.name]
    ...
    // looping through types to make type list elements, while first setting the previous type innerText to empty, or else they carry over if the previous pokemon has 2 types and the new one has 1
        ...
        pokemon.types.forEach((typeObj, typeIdx) => {
            document.getElementById(`type${typeIdx}-${pokeIdx}`).innerText = typeObj.type.name
            document.getElementById(`type${typeIdx}-${pokeIdx}`).style.backgroundColor = colors[`${typeObj.type.name}2`]
        })

        // looping through stats list to make basestats elements
        pokemon.stats.forEach((statObj, statIdx) => {
            document.getElementById(`stat${statIdx}-${pokeIdx}`).innerText = statObj.base_stat
            document.getElementById(`stat${statIdx}-${pokeIdx}`).style.backgroundColor = colors[`${pokemon.types[0].type.name}2`]
        })
        ...
        for(i = 0; i < 4; i++) {
            ...
            document.getElementById(`move${i}-${pokeIdx}`).style.backgroundColor = colors[`${pokemon.types[0].type.name}2`]
